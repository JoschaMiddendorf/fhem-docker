...
function StartFHEM {
	...
	function StopFHEM {
		...
		echo
		grep -q "Server shutdown" <(tail -f "$(date +"$LOGFILE")")					## Wait for FHEM to stop
		PrintNewLines
		echo
	...
	grep -q "Server started" <(tail -f "$(date +"$LOGFILE")")						## Wait for FHEM to start up
	PrintNewLines
	...
	if [ "$UPDATE" -eq 1 ]; then
		...
		grep -q "update finished" <(tail -f "$(date +"$LOGFILE")")					## Wait for update to finish
		PrintNewLines
		echo
